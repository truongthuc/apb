...
| Generated project structure | Generated projects include `README.md`, `AGENTS.md`, `CLAUDE.md`, and `.agent/` with agent rules, a note-linked memory index, planning, docs, architecture decisions, review history, artifacts, previews, and reviews. APB does not generate an application source or test skeleton. |
| Framework and CMS source layout | Preserve the native source boundaries of a framework, platform, or CMS such as WordPress, Laravel, or Next.js and record the actual mapping in `.agent/docs/code-organization.md`. If no source convention exists, the agent asks the owner to choose one before the first implementation task. APB does not select a `src/`, module, or test layout. APB v0.1 still requires an empty generator target, so an existing repository needs a reviewed manual migration rather than an in-place `create-apb` run. |
